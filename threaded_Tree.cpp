...
void inorder(struct Node* root)
{
    ...
    // Find the leftmost node, which will be the starting point for inorder traversal
    ...
    while (root != NULL) {
        ...
        // If right pointer is threaded, go to inorder successor
        if (root->rthread)
            root = root->right;
        else { // Otherwise, go to leftmost node in the right subtree
            root = root->right;
            while (root->lthread == false)
                root = root->left;
        }
...
{
    // If the left thread is true, the predecessor is in the left child
    if (ptr->lthread)
        return ptr->left;
